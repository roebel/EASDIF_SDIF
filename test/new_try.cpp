...
/*for exception(s)*/
    ...
    /* BEGIN: SHOULD BE CHANGED */
    generalHeaderw = SdifFWriteGeneralHeader(filew);// write on the file 'SDIF' and 4 bytes chunk size
    ...
    SDIFStream stream;
    stream.Read(file);
    std::cout << "Viewing the stream" << std::endl;
    /* Print to stdout the content of the stream */
    stream.View(file);
