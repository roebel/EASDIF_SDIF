   ...
   T SetValue(int row,int col, const T& value)
	{
	    ...
	    {
 		std::cerr<<"Error in SetValue : can not row>m_Nrows or col>m_Ncols"<<std::endl;
		return 1;
	    }
    ...
    T GetValue(int row,int col)
	{
	    if (row > m_Nrows || col > m_Ncols)
	    {
 		std::cerr<<"Error in GetValue : can not row>m_Nrows or col>m_Ncols"<<std::endl;
		return 1;
	    }
...
*/
	...
	{
	    /*
	    std::vector<T>::const_iterator ite;
	    for (ite=m_Data.begin() ; ite!= m_Data.end() ; ++ite)
            {
...
public:
    ...
    int Write(SdifFileT* file)
	{
	    ...
	    int SizeFrameW = 0;
	    int nrows=m_MatrixData.GetNbRow();
	    int ncols=m_MatrixData.GetNbCol();

	    // SdifDataTypeET  type  = SdifFCurrDataType(file);
	    m_MatrixData.Resize(nrows, ncols);
	    SizeFrameW = 0;SetValu
		...
		{
		    double value = static_cast<double>(m_MatrixData.GetValue(row,col-1));
